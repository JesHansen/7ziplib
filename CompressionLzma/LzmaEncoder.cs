...
namespace SevenZip.Compression.LZMA
{
	...
	public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
	{
        ...
        private static byte[] _gFastPos = new byte[1 << 11];
        ...
        private uint[] repDistances = new uint[Base.KNumRepDistances];
        ...
        private class LiteralEncoder
		{
			public struct Encoder2
			{
				...
				public void Create() { mEncoders = new BitEncoder[0x300]; }

				public void Init() { for (var i = 0; i < 0x300; i++) mEncoders[i].Init(); }
        ...
        private class LenEncoder
		{
            ...
            private BitTreeEncoder[] lowCoder = new BitTreeEncoder[Base.KNumPosStatesEncodingMax];
            private BitTreeEncoder[] midCoder = new BitTreeEncoder[Base.KNumPosStatesEncodingMax];
		...
		{
            private uint[] prices = new uint[Base.KNumLenSymbols << Base.KNumPosStatesBitsEncodingMax];
            private uint tableSize;
            private uint[] counters = new uint[Base.KNumPosStatesEncodingMax];
        ...
        private Optimal[] optimum = new Optimal[KNumOpts];
        private LZ.IMatchFinder matchFinder = null;
        private RangeCoder.Encoder rangeEncoder = new RangeCoder.Encoder();

        private BitEncoder[] isMatch = new BitEncoder[Base.KNumStates << Base.KNumPosStatesBitsMax];
        private BitEncoder[] isRep = new BitEncoder[Base.KNumStates];
        private BitEncoder[] isRepG0 = new BitEncoder[Base.KNumStates];
        private BitEncoder[] isRepG1 = new BitEncoder[Base.KNumStates];
        private BitEncoder[] isRepG2 = new BitEncoder[Base.KNumStates];
        private BitEncoder[] isRep0Long = new BitEncoder[Base.KNumStates << Base.KNumPosStatesBitsMax];

        private BitTreeEncoder[] posSlotEncoder = new BitTreeEncoder[Base.KNumLenToPosStates];

        private BitEncoder[] posEncoders = new BitEncoder[Base.KNumFullDistances - Base.KEndPosModelIndex];
        ...
        private uint[] matchDistances = new uint[Base.KMatchMaxLen * 2 + 2];
        ...
        private uint[] posSlotPrices = new uint[1 << (Base.KNumPosSlotBits + Base.KNumLenToPosStatesBits)];
        private uint[] distancesPrices = new uint[Base.KNumFullDistances << Base.KNumLenToPosStatesBits];
        private uint[] alignPrices = new uint[Base.KAlignTableSize];
				...
				for (uint repIndex = 0; repIndex < Base.KNumRepDistances; repIndex++)
				{
					...
					if (lenTest < numAvailableBytesFull)
					{
						...
						if (lenTest2 >= 2)
						{
							...
							var nextMatchPrice = curAndLenCharPrice + isMatch[(state2.Index << Base.KNumPosStatesBitsMax) + posStateNext].GetPrice1();
        ...
        private void SetOutStream(System.IO.Stream outStream) { rangeEncoder.SetStream(outStream); }
        private void ReleaseOutStream() { rangeEncoder.ReleaseStream(); }
        ...
        private void FillDistancesPrices()
		{
			...
			for (uint lenToPosState = 0; lenToPosState < Base.KNumLenToPosStates; lenToPosState++)
			{
				...
				for (posSlot = Base.KEndPosModelIndex; posSlot < distTableSize; posSlot++)
					posSlotPrices[st + posSlot] += ((posSlot >> 1) - 1 - Base.KNumAlignBits) << BitEncoder.KNumBitPriceShiftBits;
        ...
        private static string[] _kMatchFinderIDs = 
		{
			"BT2",
			"BT4",
		};
