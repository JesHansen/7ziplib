...
namespace SevenZip.Compression.LZMA
{
	...
	public class Decoder : ICoder, ISetDecoderProperties // ,System.IO.Stream
	{
        private class LenDecoder
		{
            ...
            private BitTreeDecoder[] mLowCoder = new BitTreeDecoder[Base.KNumPosStatesMax];
            private BitTreeDecoder[] mMidCoder = new BitTreeDecoder[Base.KNumPosStatesMax];
            private BitTreeDecoder mHighCoder = new BitTreeDecoder(Base.KNumHighLenBits);
            private uint mNumPosStates = 0;
        ...
        private class LiteralDecoder
		{
            private struct Decoder2
			{
				...
				public void Init() { for (var i = 0; i < 0x300; i++) mDecoders[i].Init(); }
        ...
        private LZ.OutWindow mOutWindow = new LZ.OutWindow();
        private RangeCoder.Decoder mRangeDecoder = new RangeCoder.Decoder();

        private BitDecoder[] mIsMatchDecoders = new BitDecoder[Base.KNumStates << Base.KNumPosStatesBitsMax];
        private BitDecoder[] mIsRepDecoders = new BitDecoder[Base.KNumStates];
        private BitDecoder[] mIsRepG0Decoders = new BitDecoder[Base.KNumStates];
        private BitDecoder[] mIsRepG1Decoders = new BitDecoder[Base.KNumStates];
        private BitDecoder[] mIsRepG2Decoders = new BitDecoder[Base.KNumStates];
        private BitDecoder[] mIsRep0LongDecoders = new BitDecoder[Base.KNumStates << Base.KNumPosStatesBitsMax];

        private BitTreeDecoder[] mPosSlotDecoder = new BitTreeDecoder[Base.KNumLenToPosStates];
        private BitDecoder[] mPosDecoders = new BitDecoder[Base.KNumFullDistances - Base.KEndPosModelIndex];

        private BitTreeDecoder mPosAlignDecoder = new BitTreeDecoder(Base.KNumAlignBits);

        private LenDecoder mLenDecoder = new LenDecoder();
        private LenDecoder mRepLenDecoder = new LenDecoder();

        private LiteralDecoder mLiteralDecoder = new LiteralDecoder();
